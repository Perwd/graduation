    ...
    <view class="scroll-view-container">
      ...
      <scroll-view class="left-scroll-view" :scroll-y="true" :style="{height:winHeight+ 'px'}">
        <block v-for="(item, index) in cateList" :key="index">
          <view :class="['left-scroll-view-item', active ===index  ? 'active' : '']" @click="activeChange(index)">
            {{item.cat_name}}
          </view>
        </block>
...
<script setup>
  ...
  function activeChange(index) {
    ...
    // console.log(cateLevel2.value[0].children)
  }
  ...
  onLoad(() => {
    ...
    console.log(sysInfo)
    // 为窗口可用高度动态赋值
    winHeight.value = sysInfo.windowHeight
